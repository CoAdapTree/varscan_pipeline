...
def checksq(rt,phase):
    ...
    if not isinstance(rt,list):
        ...
        exitneeded = True
    if len(rt) == 0:
        os.system('echo "len(sq) == 0, exiting %(thisfile)s"' % locals())
        exitneeded = True
    for s in rt:
        if not s == '':
            if 'socket' in s.lower():
                os.system('echo "socket in sq return, exiting %(thisfile)s"' % locals())
                exitneeded = True
            try:
                ...
                exitneeded = True
    if exitneeded == True:
        os.system('echo slurm screwed something up for %(thisfile)s %(phase)s, lame' % locals())
        exit()
    else:
        return rt
def getsq(phase):
